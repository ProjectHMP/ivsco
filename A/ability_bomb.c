...
int sub_4744(int iParam0, unknown uParam1)
{
    if (IS_PLAYER_PLAYING( sub_632() ))
    {
        if (IS_SCREEN_FADED_IN())
        {
            ...
            {
                if ((((IS_PLAYER_FREE_FOR_AMBIENT_TASK( sub_632() )) || (iParam0 == 5)) || (iParam0 == 4)) || ((uParam1) AND (g_U9200)))
                {
                    if (NOT sub_4840())
                    {
...
int sub_6117()
{
    ...
    {
        if ((ARE_WIDESCREEN_BORDERS_ACTIVE()) AND (sub_6151()))
        {
            return 1;
        }
