...
void sub_525()
{
    REQUEST_ANIMS( "AMB@BAR" );
    REQUEST_MODEL( 24233425 );
    if ((HAVE_ANIMS_LOADED( "AMB@BAR" )) AND (HAS_MODEL_LOADED( 24233425 )))
    {
        l_U4 = 1;
    }
    else
...
void sub_652(unknown uParam0)
{
    if (NOT (IS_CHAR_INJURED( uParam0 )))
    {
        ...
        else if (NOT (IS_CHAR_INJURED( sub_747() )))
        {
            if (IS_CHAR_SHOOTING( sub_747() ))
            {
                l_U14 = 1;
            }
        }
...
void sub_857(unknown uParam0, unknown Result)
{
    GENERATE_RANDOM_INT_IN_RANGE( uParam0, Result, ref Result );
    return Result;
}

void sub_905(unknown uParam0)
{
    switch (l_U1)
    {
        case 0:
