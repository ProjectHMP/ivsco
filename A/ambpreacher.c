...
void sub_3371()
{
    ...
    if (NOT (l_U103 == 4))
    {
        if ((HAS_CHAR_BEEN_DAMAGED_BY_CHAR( l_U113, sub_3398(), 0 )) || (IS_PED_IN_COMBAT( l_U113 )))
        {
    ...
    switch (l_U103)
    {
        ...
        if ((DOES_CHAR_EXIST( l_U114[0] )) AND (NOT (IS_CHAR_DEAD( l_U114[0] ))))
        {
            if ((DOES_CHAR_EXIST( l_U113 )) AND (NOT (IS_CHAR_DEAD( l_U113 ))))
            {
                TASK_GO_TO_CHAR( l_U113, l_U114[0], -1, 3.50000000 );
            }
        }
        else if ((DOES_CHAR_EXIST( l_U113 )) AND (NOT (IS_CHAR_DEAD( l_U113 ))))
        {
            TASK_TURN_CHAR_TO_FACE_COORD( l_U113, 584.00000000, 1403.59600000, 11.00000000 );
        }
    ...
    if (NOT ((uParam0^) == nil))
    {
        if (NOT (IS_CHAR_INJURED( (uParam0^) )))
        {
            if (NOT (IS_PED_HOLDING_AN_OBJECT( (uParam0^) )))
            {
                ...
                if ((DOES_CHAR_EXIST( (uParam0^) )) AND (NOT (IS_CHAR_DEAD( (uParam0^) ))))
                {
                    if ((DOES_CHAR_EXIST( l_U113 )) AND (NOT (IS_CHAR_DEAD( (uParam0^) ))))
                    {
