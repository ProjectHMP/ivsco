    ...
    while (true)
    {
        ...
        if (IS_PLAYER_PLAYING( sub_1257() ))
        {
            ...
            if (DOES_OBJECT_EXIST( l_U22 ))
            {
                ...
                SET_OBJECT_DYNAMIC( l_U22, 0 );
                if ((IS_WORLD_POINT_WITHIN_BRAIN_ACTIVATION_RANGE()) AND (NOT (sub_3061( l_U22 ))))
                {
...
int sub_3641()
{
    ...
    if (((((HAS_MODEL_LOADED( -1295524692 )) AND (HAS_MODEL_LOADED( -1174154857 ))) AND (HAVE_ANIMS_LOADED( "AMB@TOLLBOOTH" ))) AND (REQUEST_AMBIENT_AUDIO_BANK( "SCRIPT_AMBIENT\TOLLBOOTH" ))) AND (HAS_MODEL_LOADED( sub_3697() )))
    {
        return 1;
    }
    return 0;
