...
void sub_16()
{
    ...
    while (true)
    {
        ...
        {
            case 1:
            if (NOT (COMPARE_STRING( ref g_U8868._fU0, "" )))
            {
                if (g_U8966._fU0 < 4)
                {
                    if (NOT (HAS_THIS_ADDITIONAL_TEXT_LOADED( ref g_U8868._fU0, 7 )))
                    {
                        if (NOT sub_176())
                        {
                            REQUEST_ADDITIONAL_TEXT( ref g_U8868._fU0, 7 );
                        }
                        break;
                    }
                }
                else if (NOT (HAS_THIS_ADDITIONAL_TEXT_LOADED( ref g_U8868._fU0, 6 )))
                {
                    if (NOT sub_176())
                    {
                        REQUEST_ADDITIONAL_TEXT( ref g_U8868._fU0, 6 );
                    }
                    break;
                }
            }
            ...
            switch (g_U8966._fU0)
            {
                ...
                break;
                default:
                START_SCRIPT_CONVERSATION( 1, g_U8868._fU388 );
                break;
            }
            g_U8867 = 3;
            break;
            ...
            break;
            case 4:
            if (NOT (COMPARE_STRING( ref g_U95._fU144, "" )))
            {
                if (NOT (HAS_THIS_ADDITIONAL_TEXT_LOADED( ref g_U95._fU144, 6 )))
                {
                    if (NOT sub_176())
                    {
                        REQUEST_ADDITIONAL_TEXT( ref g_U95._fU144, 6 );
                    }
                    break;
                }
            }
...
void sub_324()
{
    ...
    {
        if ((NOT (COMPARE_STRING( ref g_U8972[I], "END" ))) AND (NOT (COMPARE_STRING( ref g_U8972[I], "" ))))
        {
            iVar31 = 1;
            cVar11 = {g_U8972[I]};
            ...
            if (DOES_TEXT_LABEL_EXIST( ref uVar7 ))
            {
                if (iVar32 >= iVar36)
                {
                    ...
                    sub_1054( iVar33, iVar34, iVar35, ref cVar15, ref g_U8972[I] );
                    if (bVar37)
                    {
                        return;
                    }
            ...
            {
                sub_1337( ref uVar3, ref g_U8972[I], iVar31, -1, -1 );
...
void sub_1054(int iParam0, int iParam1, unknown uParam2, unknown uParam3, unknown uParam4)
{
    switch (g_U8867)
    {
        ...
        if (iParam0 < 9)
        {
            g_U8868._fU344[iParam0] = 1;
        }
        if (iParam1 < 9)
        {
            g_U8868._fU344[iParam1] = 1;
        }
...
void sub_2465()
{
    ...
    for ( I = 0; I <= 8; I++ )
    {
        if (g_U8868._fU344[I])
        {
            ADD_NEW_CONVERSATION_SPEAKER( I, g_U8868._fU16[I]._fU0, ref g_U8868._fU16[I]._fU4 );
        }
    }
    return;
}
